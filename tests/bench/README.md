...
The **representative** tier uses production-to-production dominance vs each applicable
baseline (smaller at ≥ quality, or better at ≤ size) → PASS; baseline dominates or a hard
constraint breached (quality < 50, time > 3 s static / 10 s animated, RSS > 512 MB) →
FAIL; a genuine trade-off → INCOMPARABLE (record a decision in the PR).
